...
void resetRally() {
  ...
  ball.cpuTargetX =
      rightSideServes
          ? ball.x + (0.2 + rand() / (float)RAND_MAX) * PLAYER_RADIUS
          : 160 / 2 + 160 / 4;
...
void updateBall(Ball *ball) {
  ...
  if (ball->y > 160 - GROUND_HEIGHT - BALL_RADIUS && ball->velY > 0) {
    ...
    pauseTime =
        (scoreLeft >= SCORE_LIMIT || scoreRight >= SCORE_LIMIT) ? 120 : 60;
  ...
  for (int ii = 0; ii < playerCount; ++ii) {
    ...
    float minDist2 =
        (PLAYER_RADIUS + BALL_RADIUS) * (PLAYER_RADIUS + BALL_RADIUS);
    if (dist2 < minDist2) {
      float dist = sqrt(dist2);
      ball->x = player->x + (PLAYER_RADIUS + BALL_RADIUS) * dx / dist;
      ball->y = player->y + (PLAYER_RADIUS + BALL_RADIUS) * dy / dist;

      ball->velX = PLAYER_MASS * player->velX +
                   BALL_BOUNCE * dx / (PLAYER_RADIUS + BALL_RADIUS);
      ball->velY = PLAYER_MASS * player->velY +
                   BALL_BOUNCE * dy / (PLAYER_RADIUS + BALL_RADIUS);
      ...
      if (ball->velY > 1 && ball->y < 160 - GROUND_HEIGHT - 2 * PLAYER_RADIUS) {
        ...
        tone(ball->x < 160 / 2 ? 65 : 62, 4 << 8, 100,
  ...
  if (gameMode == 0 && velYChanged) {
    ...
    if (x < 160 / 2) {
      // The ball will land on the left (human) side, so move to the center of
      // the right side
      x = 160 / 2 + 160 / 4;
...
void updateAIPlayer(Player *player) {
  ...
  if (ball.x > 160 / 2 && ball.y > 160 - GROUND_HEIGHT - 5 * PLAYER_RADIUS &&
      fabs(ball.velX) < 1 && ball.velY < 2) {
    gamepad |= BUTTON_1;
  }
...
void drawPlayer(const Player *player, int playerIdx) {
  ...
  *DRAW_COLORS = alternateColor ? 0x1230 : 0x1340;
  blit(imgPlayer, player->x - PLAYER_RADIUS, player->y - PLAYER_RADIUS,
       imgPlayerWidth, imgPlayerHeight,
       imgPlayerFlags | (rightSide ? BLIT_FLIP_X : 0));
...
void drawBall(const Ball *ball) {
  *DRAW_COLORS = 0x4230;
  blit(imgBall, ball->x - BALL_RADIUS, ball->y - BALL_RADIUS, imgBallWidth,
       imgBallHeight, imgBallFlags);
}

void drawNet() {
  rect(160 / 2 - NET_WIDTH / 2, 160 - NET_HEIGHT - GROUND_HEIGHT, NET_WIDTH,
       NET_HEIGHT);
}

void drawGround() { rect(0, 160 - GROUND_HEIGHT, 160, GROUND_HEIGHT); }

void drawHUD() {
  // Draw filled balls
  *DRAW_COLORS = 0x231;
  for (int ii = 0; ii < scoreLeft; ++ii) {
    blit(imgBall, 1 + ii * (imgBallWidth + 1), 1, imgBallWidth, imgBallHeight,
         imgBallFlags);
  }
  for (int ii = 0; ii < scoreRight; ++ii) {
    blit(imgBall, 160 - (ii + 1) * (imgBallWidth + 1), 1, imgBallWidth,
         imgBallHeight, imgBallFlags);
  }
...
void drawTitle() {
  ...
  *DRAW_COLORS = 0x40;
  blit(imgArrow, menuX - imgArrowWidth - 4, menuY + gameMode * 8, imgArrowWidth,
       imgArrowHeight, imgArrowFlags);
